...
const Slider = ({ carouselPromos }: CarouselImagesData): React.JSX.Element => {
  ...
  const showNextImage = (): void => {
    setCurrentImageIndex((index) => {
      if (index === carouselPromos.length - 1) {
        return 0;
      } else {
        return index + 1;
      }
  ...
  const showPreviousImage = (): void => {
    setCurrentImageIndex((index) => {
      if (currentImageIndex === 0) {
        return carouselPromos.length - 1;
      } else {
        return index - 1;
      }
    });
  ...
  const content: React.JSX.Element = (
    ...
    >
      <div className="w-full h-full flex flex-row overflow-hidden">
        {Object.values(carouselPromos).map(
          (img: CarouselImageData, index: number) => (
            <img
              key={index}
      ...
      <div className="w-full absolute flex flex-col top-2/3 gap-4 bg-gray-900 bg-opacity-35 p-4">
        <h2 className="text-center font-semibold text-2xl md:text-3xl lg:text-4xl text-white">
          {carouselPromos[currentImageIndex].caption.heading}
        </h2>
        <p className="text-center font-medium text-base md:text-lg text-white">
          {carouselPromos[currentImageIndex].caption.description}
        </p>
      </div>
      <button
      ...
      <div className="w-full absolute bottom-8 flex justify-center">
        <div className="flex gap-2">
          {carouselPromos.map((_: CarouselImageData, index: number) => (
            <button
              key={index}
