...
const UserOrderItem = ({item}: OrderItemable): React.JSX.Element => {
    ...
    const content: React.JSX.Element = (
      <div
        className={`grid grid-cols-5 justify-evenly ${themeClasses.textClass}`}
      >
        ...
        <div className="col-span-1 flex flex-col justify-center items-center gap-1">
          ...
          <p aria-label={`Quantity of ${item.itemDetails.name}`}>
            {item.itemQuantity}
          </p>
        </div>
        <div className="col-span-1 flex flex-col justify-center items-center gap-1">
          <h4 className="text-lg font-medium ">Item Price</h4>
          <p aria-label={`Price of ${item.itemDetails.name}`}>
            {item.itemPrice}
          </p>
        </div>
      </div>
